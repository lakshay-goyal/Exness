...
```
                 ...
                 ▼                                ▼                            ▼
   ┌──────────────────────-┐        ┌──────────────────────────┐   ┌─────────────────────┐
   │  Terraform (infra/aws)│        │  docker build & push ×5  │   │  SSM Run Command    │
...
│                                                                                          │
│   ┌────────────-─┐    ┌──────────────────────────────────────────────────────────────┐   │
│   │ ECR ×5 repos │    │  VPC 10.0.0.0/16 — public subnet, NO NAT (saves $32/mo)      │   │
│   │ exness-      │    │                                                              │   │
│   │  backend     │    │   ┌────────────────── EC2 t3.medium ──────────────────┐      │   │
│   │  engine      │───▶│   │  Docker Compose (/opt/exness)                     │      │   │
│   │  snapshotting│    │   │                                                   │      │   │
│   │  dbstorage   │    │   │  ┌─────────┐ ┌────────┐                           │      │   │
│   │  web         │    │   │  │ backend │ │  web   │  ◄── public               │      │   │
│   │              │    │   │  │  :8000  │ │ :3001  │      ports                │      │   │
│   └──────-───────┘    │   │  └────┬────┘ └────────┘                           │      │   │
│                       │   │       │                                           │      │   │
│   ┌─────────────--┐    │   │  ┌────┴────┐ ┌──────────────┐ ┌───────────┐      │      │   │
...
Security group only opens **3001, 8000** (plus 80/443 for nginx). No port 22 — shell access is
...
| --- | --- | --- |
| Compute | 1× EC2 t3.medium ≈ $30/mo | 5× Fargate tasks ≈ $50/mo + ALB $18/mo |
...
(instance boot + 5 image builds + first image pulls). Service URLs appear in
...
| `NEXT_PUBLIC_BACKEND_URL` | Terraform output → docker **build arg** in CI (baked into web bundle) | web image at build time |
...
2. **Backups**: schedule EBS snapshots (AWS Backup or DLM) — DB data lives on
   the instance volume.
3. **Bootstrap role** uses `AdministratorAccess`; scope down once resources stabilize.
