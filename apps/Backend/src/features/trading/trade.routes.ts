...
tradeRouter.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      const orderResult = parseStreamMessage<Record<string, any>>(
        result.message,
        {},
      );
...
tradeRouter.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (!orderId) {
      ...
      return res
        .status(400)
        .json({ error: "Missing required parameters: orderId" });
    }
    ...
    try {
      ...
      const orderData = parseStreamMessage<Record<string, any>>(
        response.message,
        {},
      );
