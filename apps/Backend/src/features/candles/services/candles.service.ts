...
class CandlesService {
  ...
  private async retrieveDataFromTrades(
    symbol: string,
    interval: string,
    from: string,
    to: string,
    ...
    if (result.rows.length === 0) {
      const queryNoDate = `
        ...
        LIMIT 500;
      `;
      const resultNoDate = await client
        .getClient()
        .query(queryNoDate, [symbol]);
  ...
  private async retrieveData(
    ...
    try {
      ...
      const queryNoDate = `
        ...
        LIMIT 500;
      `;
      const resultNoDate = await client.getClient().query(queryNoDate, [
        symbol,
      ]);
      if (resultNoDate.rows.length > 0) {
        return resultNoDate.rows.reverse();
      ...
      const fallbackData = await this.retrieveDataFromTrades(
        symbol,
        interval,
        from,
        to,
