...
- `/` — landing page (includes the system architecture and the interactive OpenAPI reference for unauthenticated visitors)
...
Everything — all five services plus PostgreSQL, Redis, and MongoDB — runs in Compose. Migrations run automatically via the `db-migrate` one-shot container.
